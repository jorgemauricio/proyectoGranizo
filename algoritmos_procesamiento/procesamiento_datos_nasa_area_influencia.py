...
def main():
    ...
    for nombre in data["Nombre"].unique():
        ...
        dataTemp["diff_previa"] = abs(dataTemp["areaInfluencia2"] - dataTemp["areaInfluencia2"].shift(-1))

        # crear columna de diferencia tiempo posterior
        dataTemp["diff_posterior"] = abs(dataTemp["areaInfluencia2"] - dataTemp["areaInfluencia2"].shift(1))

        # crear columna de siguiente valor
        dataTemp["areaInfluencia_posterior_1"] = dataTemp["areaInfluencia2"].shift(1)

        # crear columna de siguiente valor
        dataTemp["areaInfluencia_posterior_2"] = dataTemp["areaInfluencia2"].shift(2)
        ...
        dataTemp["validacionEvento"] = dataTemp.apply(lambda x: validacionSeguimiento(x["areaInfluencia2"], x["areaInfluencia_posterior_1"], x["areaInfluencia_posterior_2"]), axis=1)

        # eliminar datos nulos
        dataTemp = dataTemp.fillna(value=0)

        # evitar columnas ajenas
        dataTemp = dataTemp[['Canon', 'Estado', 'Nombre', 'Long', 'Lat', 'Year', 'Month', 'Day', 'Hour', 'RainIMR','areaInfluencia2', 'areaInfluencia_posterior_1', 'areaInfluencia_posterior_2', 'diff_previa', 'diff_posterior', 'Rango_previo', 'Rango_posterior', 'validacionEvento', 'Fecha', 'Hora']]
    ...
    nombreArchivoFinal = "{}/data/Resultado_area_influencia_3.csv".format(path)
