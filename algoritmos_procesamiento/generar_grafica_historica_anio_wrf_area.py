...
def main():
    ...
    # nombre del archivo
    nombreTemporalArchivo = "{}/data/Resultado_wrf_area_influencia_2.csv".format(path)
    ...
    for i in data["Nombre"].unique():
        ...
        # ciclo por Año
        for j in dataTemp["Year"].unique():
            dataFecha = dataTemp.loc[data["Year"] == j]
            dataFecha.index = dataFecha["Fecha"]
            dataFecha.index = pd.to_datetime(dataFecha.index)
            ...
            y = np.array(dataFecha["areaInfluencia2"])
            ...
            # guardar gráfica
            nombreTemporalGrafica = "data/graphs/anuales/{}_{}_historica_wrf_af.png".format(j,i)
